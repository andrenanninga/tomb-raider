...
var structs = {
  ...
  tr2_animation: jBinary.Type({
    read: function(context) {
      ...
      // Unknown
      this.binary.skip(2);
      data.speed = this.binary.read('int16');
      data.accelLow = this.binary.read('uint16');
      data.accelHigh = this.binary.read('int16');
      ...
      data.AnimCommand = this.binary.read('uint16');

      data.NumFrames = data.FrameEnd - data.FrameStart + 1;
      data.NumKeys = parseInt((data.NumFrames - 1) / data.FrameRate) + 1;

      data.FPS = 30;
      data.Length = data.NumFrames / data.FPS;

      if((data.NumFrames - 1) % data.FrameRate) { 
        data.NumKeys += 1; 
      }

      _.times(data.NumKeys, function(i) {

        var frameIndex = data.FrameOffset / 2 + i * data.FrameSize;
        var frameIndexStart = frameIndex;
        var frame = {};

        frame.BB1x = context.Frames[frameIndex++];
        frame.BB2x = context.Frames[frameIndex++];

        frame.BB1y = context.Frames[frameIndex++];
        frame.BB2y = context.Frames[frameIndex++];
        
        frame.BB1z = context.Frames[frameIndex++];
        frame.BB2z = context.Frames[frameIndex++];

        frame.x = context.Frames[frameIndex++];
        frame.y = context.Frames[frameIndex++];
        frame.z = context.Frames[frameIndex++];

        frame.Meshes = [];

        while(true) {

          if(frameIndex - frameIndexStart >= data.FrameSize) {
            break;
          }

          var rotation = [0, 0, 0];
          var rotation1 = context.Frames[frameIndex++];

          if(rotation1 & 0xC000) {
            // single angle
            if((rotation1 & 0x8000) && (rotation1 & 0x4000)) {
              rotation[2] = (rotation1 & 0x03FF);
            }
            else if((rotation1 & 0x4000)) {
              rotation[0] = (rotation1 & 0x03FF);
            }
            else {
              rotation[1] = (rotation1 & 0x03FF);
            }
          }
          else {
            ...
            // three angles
            var rotation2 = context.Frames[frameIndex++];
            rotation[0] = (rotation1 & 0x3FF0) >> 4;
            rotation[1] = ((rotation1 & 0x000f) << 6) | ((rotation2 & 0xFC00) >> 10);
            rotation[2] = (rotation2 & 0x03FF);
          }

          rotation[0] = Math.round(rotation[0] * 360 / 1024);
          rotation[1] = Math.round(rotation[1] * 360 / 1024);
          rotation[2] = Math.round(rotation[2] * 360 / 1024);
